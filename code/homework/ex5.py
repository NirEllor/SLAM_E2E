...
def q5_1(db):
    """5.1: Analyzes single track reprojection error using GTSAM bundle adjustment."""
...
def _build_bundle_graph_for_window(db, window_frames, K_gtsam):
    """Builds factor graph with camera poses and landmarks for bundle window."""
    ...
    R_start, t_start = camera_poses[window_frames[0]]
    ...
    for t_id in landmarks_in_window:
        ...
        if len(temporary_factors) >= 2:
            ...
            for factor in temporary_factors:
                graph.add(factor)
    return graph, initial_estimate, landmarks_added, window_frames[0]


def _find_worst_error_factor(graph, initial_estimate):
    """Identifies the factor with maximum initial reprojection error."""
    max_init_error = -1.0
    worst_factor = None
    worst_factor_idx = -1
    worst_c_key = None
    ...
    return worst_factor, worst_factor_idx, worst_c_key, worst_q_key, max_init_error


def _print_bundle_diagnostics(graph, initial_estimate, result, worst_factor, worst_c_key, worst_q_key, K_gtsam, db, max_init_error):
    """Prints pre/post optimization bundle adjustment diagnostics."""
    num_factors = graph.size()
    num_projection_factors = num_factors - 1
    ...
    if worst_factor is not None:
        ...
        print("-"*80)
        print(f"Camera Frame Key  (c):           symbol('c', {c_index})")
        print(f"Landmark Point Key (q):          symbol('q', {q_index})")
        ...
        print(f"Measurement  — Left X: {measured.uL():8.2f}  |  Right X: {measured.uR():8.2f}  |  Y: {measured.v():8.2f}")
    ...
    print(f"Error Reduction:  {initial_total_error:.4f}  -->  {final_total_error:.4f}  " +
    ...
    if worst_factor is not None:
        ...
        print(f"Projection improved from {max_init_error:.6f} to {final_factor_error:.6f}")


def q5_3(db):
    """5.3: Optimizes first bundle adjustment window using GTSAM."""
    print("\n" + "="*80)
    print("SECTION 5.3: FIRST BUNDLE ADJUSTMENT WINDOW (10 FRAMES)")
    print("="*80)
    K_gtsam = lib.init_gtsam_stereo_calibration()
    window_frames = list(range(0, 10))
    print(f"Bundle Window spans from Frame 0 to Frame 9 (Total 10 frames)")
    graph, initial_estimate, landmarks_added, start_frame = _build_bundle_graph_for_window(db, window_frames, K_gtsam)
    print(f"Landmarks inserted into graph: {len(landmarks_added)}")
    worst_factor, worst_factor_idx, worst_c_key, worst_q_key, max_init_error = _find_worst_error_factor(graph, initial_estimate)
    result = gtsam.LevenbergMarquardtOptimizer(graph, initial_estimate).optimize()
    _print_bundle_diagnostics(graph, initial_estimate, result, worst_factor, worst_c_key, worst_q_key, K_gtsam, db, max_init_error)
...
def _compute_pnp_baseline_distances(db, keyframes):
    """Computes baseline camera distances for scale correction during sliding window BA."""
    ...
    return bundle_windows, np.median(pnp_distances)


def _solve_bundle_windows_sequentially(db, bundle_windows, median_pnp_dist):
    """Solves bundle adjustment for each window with scale correction and fallback PnP."""
    global_keyframe_poses = {bundle_windows[0][0]: gtsam.Pose3()}
    ...
    return global_keyframe_poses, global_landmarks_dict, last_bundle_result, failed_bundles


def q5_4(db):
    """5.4: Runs full sliding window bundle adjustment over keyframe sequence."""
    print("\n" + "="*80)
    ...
    bundle_windows, median_pnp_dist = _compute_pnp_baseline_distances(db, keyframes)
    print(f"Number of bundle windows: {len(bundle_windows)}")
    global_keyframe_poses, global_landmarks_dict, last_bundle_result, failed_bundles = _solve_bundle_windows_sequentially(db, bundle_windows, median_pnp_dist)
